...
class Redstone_TradeController extends Zend_Controller_Action {
	private $model;
	private $base;
	private $table;
	private $member;
	...
	public function init() {
		...
		require_once($root_dir . 'models/TradeModel.php');
		$this->model = new Redstone_TradeModel ();

		$this->table = 'trade';
		$this->imagePath = dirname ( dirname ( dirname ( dirname ( dirname ( __FILE__ ) ) ) ) ) . '/data/images/redstone//trade';
...
//		$this->view->name = htmlspecialchars ($member->name);
		$this->view->title = htmlspecialchars ( '作成中' );
	}
