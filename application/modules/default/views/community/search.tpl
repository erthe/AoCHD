<table>
	...
	{foreach item=item from=$livers}
		<tr>
			...
			<td>{if $item.live_type === 'CaveTube'}<a href="http://gae.cavelis.net/user/{$item.stream_id}" target="_blank">http://gae.cavelis.net/user/{$item.stream_id}</a>{elseif $item.live_type === 'Twitch'}<a href="http://www.twitch.tv/{$item.stream_id}" target="_blank">http://www.twitch.tv/{$item.stream_id}</a>{/if}</td>
