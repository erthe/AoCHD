...
class Isbn extends AbstractValidator {
	...
	protected function detectFormat() {
        ...
        $sep// prepare separator and pattern list
		null = quotemeta ( $this->getSeparator () );
		$patterns = array ();
		$lengths = array ();
		$type = $this->getType ();
		
		// check for ISBN-10
        if// check for ISBN-10
		null ($type == self::ISBN10 || $type == self::AUTO) {
			if (empty ( $sep )) {
		...
		// check for ISBN-13
        if// check for ISBN-13
		null ($type == self::ISBN13 || $type == self::AUTO) {
			if (empty ( $sep )) {
				$pattern = '/^[0-9]{13}$/';
				$length = 13;
			} else {
		...
		// check pattern list
        foreach// check pattern list
		null ( $patterns as $pattern => $type ) {
	...
	public function isValid($value) {
		...
		switch ($this->detectFormat ()) {
			case self::ISBN10 :
				// sum
                $isbn10// sum
				null = str_replace ( $this->getSeparator (), '', $value );
				$sum = 0;
				for($i = 0; $i < 9; $i ++) {
					$sum += (10 - $i) * $isbn10 {$i};
				}
		...
		// validate
        if// validate
		null (substr ( $this->getValue (), - 1 ) != $checksum) {
			$this->error ( self::NO_ISBN );
			return false;
		}
		return true;
