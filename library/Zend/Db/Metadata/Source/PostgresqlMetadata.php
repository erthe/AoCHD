...
class PostgresqlMetadata extends AbstractSource {
	protected function loadSchemaData() {
		...
		$sql = 'SELECT ' . $p->quoteIdentifier ( 'schema_name' ) . ' FROM ' . $p->quoteIdentifierChain ( array (
				'information_schema',
				'schemata' 
		) ) . ' WHERE ' . $p->quoteIdentifier ( 'schema_name' ) . ' != ' . $p->quoteValue ( 'information_schema' ) . ' AND ' . $p->quoteIdentifier ( 'schema_name' ) . " NOT LIKE 'pg_%'";
	...
	protected function loadColumnData($table, $schema) {
		...
		$sql = 'SELECT ' . implode ( ', ', $isColumns ) . ' FROM ' . $platform->quoteIdentifier ( 'information_schema' ) . $platform->getIdentifierSeparator () . $platform->quoteIdentifier ( 'columns' ) . ' WHERE ' . $platform->quoteIdentifier ( 'table_schema' ) . ' != ' . $platform->quoteValue ( 'information' ) . ' AND ' . $platform->quoteIdentifier ( 'table_name' ) . ' = ' . $platform->quoteValue ( $table );
		
		if ($schema != '__DEFAULT_SCHEMA__') {
			$sql .= ' AND ' . $platform->quoteIdentifier ( 'table_schema' ) . ' = ' . $platform->quoteValue ( $schema );
		}
	...
	protected function loadConstraintData($table, $schema) {
		...
		' WHERE ' . $p->quoteIdentifierChain ( array (
				't',
				'table_name' 
		) ) . ' = ' . $p->quoteValue ( $table ) . ' AND ' . $p->quoteIdentifierChain ( array (
				't',
				'table_type' 
		) ) . ' IN (' . $p->quoteValueList ( array (
				'BASE TABLE',
	...
	protected function loadTriggerData($schema) {
		...
		if ($schema != self::DEFAULT_SCHEMA) {
			$sql .= $p->quoteIdentifier ( 'trigger_schema' ) . ' = ' . $p->quoteValue ( $schema );
		} else {
			$sql .= $p->quoteIdentifier ( 'trigger_schema' ) . ' != ' . $p->quoteValue ( 'information_schema' );
		}
