...
class Form extends Fieldset implements FormInterface {
	...
	public function setBindOnValidate($bindOnValidateFlag) {
		if (! in_array ( $bindOnValidateFlag, array (
			...
			throw new Exception\InvalidArgumentException ( sprintf ( '%s expects the flag to be one of %s::%s or %s::%s', __METHOD__, get_called_class (), 'BIND_ON_VALIDATE', get_called_class (), 'BIND_MANUAL' ) );
		}
