 ...
 *
 * @package recAnalyst
 * @version $Id: RecAnalyst.php 26 2013-11-30 09:26:02Z mittermeyer $
...
class RecAnalyst {
    ...
    */
	...
	$this->headerStream->readFloat($subVersion);
        ...
        switch ($version) {
            ...
            case RecAnalystConst::VER_94:
           	if ($subVersion > 11.76) {
	            $this->gameInfo->_gameVersion = GameVersion::AOC14;
        	} else {
		    $this->gameInfo->_gameVersion = GameVersion::AOC13;
            	}
                break;
	...
	if ($this->gameInfo->_gameVersion === GameVersion::AOC14) {
		if ($subVersion >= 11.97) {
                	$this->gameInfo->_gameSubVersion = '3.x';
            	} else if ($subVersion >= 12.3) {
                	$this->gameInfo->_gameSubVersion = '4.x';
            	} else {
        ...
        if ($scenario_header_pos > 0) {
            ...
            if ($original_sc_filename != '') {
                $original_sc_filename = $original_sc_filename;
                    ...
                    elseif ($command == -1) {
                        ...
                        for ($i = 0; $i < $this->players->count(); $i++) {
                            ...
                            if ($player->feudalTime != 0 && $player->feudalTime < $time_cnt && $age_flag[$i] < 1) {
                                ...
                                $chatmessage->msg = sprintf('%s が領主の時代に進化しました', $player->name);
                                $this->ingameChatMessages->add($chatmessage);

                                // see reading pre-game messages, 0 indicates game's message
                                $this->ingameChat[] = sprintf('%d@#0%s が領主の時代に進化しました', $player->feudalTime, $player->name);
                                $age_flag[$i] = 1;
                            }
                            if ($player->castleTime != 0 && $player->castleTime < $time_cnt && $age_flag[$i] < 2) {
                                $chatmessage = new ChatMessage();
                                $chatmessage->time = $player->castleTime;
                                $chatmessage->msg = sprintf('%s が城主の時代に進化しました', $player->name);
                                $this->ingameChatMessages->add($chatmessage);

                                $this->ingameChat[] = sprintf('%d@#0%s が城主の時代に進化しました', $player->castleTime, $player->name);
                                $age_flag[$i] = 2;
                            }
                            if ($player->imperialTime != 0 && $player->imperialTime < $time_cnt && $age_flag[$i] < 3) {
                                $chatmessage = new ChatMessage();
                                $chatmessage->time =$player->imperialTime;
                                $chatmessage->msg = sprintf('%s が帝王の時代に進化しました', $player->name);
                                $this->ingameChatMessages->add($chatmessage);

                                $this->ingameChat[] = sprintf('%d@#0%s が帝王の時代に進化しました', $player->imperialTime, $player->name);
                                $age_flag[$i] = 3;
                            }
                        }
                    ...
                    switch ($command) {
                        case 0x0B: // player resign
                            ...
                            if (($player = $this->players->getPlayerByIndex($player_index)) && $player->resignTime == 0) {
                                ...
                                $chatmessage = new ChatMessage();
                                $chatmessage->time = $time_cnt;
                                $chatmessage->msg = sprintf('%s がゲームを放棄しました', $player->name);
                                $this->ingameChatMessages->add($chatmessage);

                                $this->ingameChat[] = sprintf('%d@#0%s がゲームを放棄しました', $player->resignTime, $player->name);
                            }
...
*/
        ...
        $max_username_height = 0;
        $font = dirname(__FILE__) . '/APJapanesefontT.ttf';
	$size = 12;
        $real_cnt = 0;
        foreach ($this->players as $player) {
            if (empty($player->researches)) {
            ...
            $bbox = imagettfbbox($size, 0, $font, $player->name);
            if($bbox[4] > $max_username_width) $max_username_width = $bbox[4];
            if($bbox[3] > $max_username_height) $max_username_height = $bbox[3];
            $real_cnt++;
        }
...
*/
            $dst_y = $idx * ($rh + $spacing) + $padding + round($max_username_height / 2); $dst_x = 0 + $padding;
            $idx++;
...
*/
            imagettftext($gd, $size, 0, $dst_x + 1, $dst_y + $size + 1, $black, $font, $player->name);
            imagettftext($gd, $size, 0, $dst_x, $dst_y + $size, $color, $font, $player->name);
        }
