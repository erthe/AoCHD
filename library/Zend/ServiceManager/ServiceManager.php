...
class ServiceManager implements ServiceLocatorInterface {
	...
	public function setShareByDefault($shareByDefault) {
		if ($this->allowOverride === false) {
			throw new Exception\RuntimeException ( sprintf ( '%s: cannot alter default shared service setting; container is marked immutable (allow_override is false)', __METHOD__ ) );
		}
	...
	public function addInitializer($initializer, $topOfStack = true) {
		if (! is_callable ( $initializer ) && ! $initializer instanceof InitializerInterface) {
			if (! is_string ( $initializer ) || ! $this->isSubclassOf ( $initializer, __NAMESPACE__ . '\InitializerInterface' )) {
	...
	public function setService($name, $service) {
		...
		if ($this->has ( $cName, false )) {
			if ($this->allowOverride === false) {
				throw new Exception\InvalidServiceNameException ( sprintf ( '%s: A service by the name "%s" or alias already exists and cannot be overridden, please use an alternate name.', __METHOD__, $name ) );
			}
			$this->unregisterService ( $cName );
		}
	...
	public function setShared($name, $isShared) {
		...
		if (! isset ( $this->invokableClasses [$cName] ) && ! isset ( $this->factories [$cName] ) && ! $this->canCreateFromAbstractFactory ( $cName, $name )) {
			throw new Exception\ServiceNotFoundException ( sprintf ( '%s: A service by the name "%s" was not found and could not be marked as shared', __METHOD__, $name ) );
		}
	...
	public function get($name, $usePeeringServiceManagers = true) {
		...
		if ($instance === null && ! is_array ( $instance )) {
			...
			throw new Exception\ServiceNotFoundException ( sprintf ( '%s was unable to fetch or create an instance for %s', __METHOD__, $name ) );
		}
	...
	public function canCreate($name, $checkAbstractFactories = true) {
		...
		if (isset ( $this->invokableClasses [$cName] ) || isset ( $this->factories [$cName] ) || isset ( $this->aliases [$cName] ) || isset ( $this->instances [$cName] )) {
			return true;
		}
	...
	public function canCreateFromAbstractFactory($cName, $rName) {
		...
		foreach ( $this->abstractFactories as $index => $abstractFactory ) {
			...
			if (isset ( $this->pendingAbstractFactoryRequests [get_class ( $abstractFactory )] ) && $this->pendingAbstractFactoryRequests [get_class ( $abstractFactory )] == $rName) {
				return false;
			}
	...
	protected function createFromInvokable($canonicalName, $requestedName) {
		...
		if (! class_exists ( $invokable )) {
			throw new Exception\ServiceNotFoundException ( sprintf ( '%s: failed retrieving "%s%s" via invokable class "%s"; class does not exist', __METHOD__, $canonicalName, ($requestedName ? '(alias: ' . $requestedName . ')' : ''), $invokable ) );
		}
