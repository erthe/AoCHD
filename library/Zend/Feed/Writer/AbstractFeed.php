...
class AbstractFeed {
	...
	public function addAuthor(array $author) {
		...
		if (isset ( $author ['uri'] )) {
			if (empty ( $author ['uri'] ) || ! is_string ( $author ['uri'] ) || ! Uri\UriFactory::factory ( $author ['uri'] )->isValid ()) {
	...
	public function setId($id) {
		if ((empty ( $id ) || ! is_string ( $id ) || ! Uri\UriFactory::factory ( $id )->isValid ()) && ! preg_match ( "#^urn:[a-zA-Z0-9][a-zA-Z0-9\-]{1,31}:([a-zA-Z0-9\(\)\+\,\.\:\=\@\;\$\_\!\*\-]|%[0-9a-fA-F]{2})*#", $id ) && ! $this->_validateTagUri ( $id )) {
	...
	public function addCategory(array $category) {
		...
		if (isset ( $category ['scheme'] )) {
			if (empty ( $category ['scheme'] ) || ! is_string ( $category ['scheme'] ) || ! Uri\UriFactory::factory ( $category ['scheme'] )->isValid ()) {
