...
class Serializer extends AbstractPlugin {
	...
	public function attach(EventManagerInterface $events, $priority = 1) {
		...
		// read
		$handles [] = $events->attach ( 'getItem.post', array (
				$this,
				'onReadItemPost' 
		), $postPriority );
		$handles [] = $events->attach ( 'getItems.post', array (
				$this,
				'onReadItemsPost' 
		), $postPriority );
		
		// write
		$handles [] = $events->attach ( 'setItem.pre', array (
				$this,
				'onWriteItemPre' 
		), $prePriority );
		$handles [] = $events->attach ( 'setItems.pre', array (
				$this,
				'onWriteItemsPre' 
		), $prePriority );
		
		$handles [] = $events->attach ( 'addItem.pre', array (
				$this,
				'onWriteItemPre' 
		), $prePriority );
		$handles [] = $events->attach ( 'addItems.pre', array (
				$this,
				'onWriteItemsPre' 
		), $prePriority );
		
		$handles [] = $events->attach ( 'replaceItem.pre', array (
				$this,
				'onWriteItemPre' 
		), $prePriority );
		$handles [] = $events->attach ( 'replaceItems.pre', array (
				$this,
				'onWriteItemsPre' 
		), $prePriority );
		
		$handles [] = $events->attach ( 'checkAndSetItem.pre', array (
				$this,
				'onWriteItemPre' 
		), $prePriority );
		
		// increment / decrement item(s)
		$handles [] = $events->attach ( 'incrementItem.pre', array (
				$this,
				'onIncrementItemPre' 
		), $prePriority );
		$handles [] = $events->attach ( 'incrementItems.pre', array (
				$this,
				'onIncrementItemsPre' 
		), $prePriority );
		
		$handles [] = $events->attach ( 'decrementItem.pre', array (
				$this,
				'onDecrementItemPre' 
		), $prePriority );
		$handles [] = $events->attach ( 'decrementItems.pre', array (
				$this,
				'onDecrementItemsPre' 
		), $prePriority );
		
		// overwrite capabilities
		$handles [] = $events->attach ( 'getCapabilities.post', array (
				$this,
